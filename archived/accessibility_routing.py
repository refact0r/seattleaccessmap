...
for u, v, key, data in G_proj.edges(keys=True, data=True):
    ...
    if nearby_indices:
        ...
        for _, barrier in nearby_barriers.iterrows():
            dist = (
                np.sqrt(
                    (barrier["lat"] - mid_lat) ** 2 + (barrier["lon"] - mid_lng) ** 2
                )
                * METERS_PER_DEGREE
            )
...
try:
    ...
    print(f"\nComparison:")
    print(
        f"  Extra distance: {accessible_length - standard_length:.0f} m ({((accessible_length/standard_length - 1)*100):.1f}%)"
    )
    print(
        f"  Barrier reduction: {standard_barrier_cost - accessible_barrier_cost:.1f} ({((1 - accessible_barrier_cost/max(standard_barrier_cost, 0.01))*100):.1f}%)"
    )
    ...
    print(
        "✓ Routes saved to output/route_accessible.geojson and output/route_standard.geojson"
    )
